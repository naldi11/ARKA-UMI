...
@section('title', 'Upload Target Jurnal')
...
<div class="max-w-2xl mx-auto">
    <div class="mb-7">
        <nav class="flex items-center gap-2 text-xs text-slate-400 mb-2 uppercase tracking-widest font-semibold">
            ...
            <span class="text-blue-700 font-bold">Upload Target Jurnal</span>
        </nav>
        <h1 class="text-2xl font-black tracking-tight text-slate-900">Upload Target Jurnal</h1>
        <p class="text-slate-500 text-sm mt-1">Unggah file bukti target jurnal publikasi Anda (format PDF, maks. 10 MB).</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-7">
        <div class="flex items-center gap-3 mb-6 pb-5 border-b border-slate-100">
            <div class="w-10 h-10 {{ $thesis->doc_target_jurnal ? 'bg-emerald-100' : 'bg-slate-100' }} rounded-xl flex items-center justify-center">
                <span class="material-symbols-outlined {{ $thesis->doc_target_jurnal ? 'text-emerald-600' : 'text-slate-500' }} text-[22px]" style="{{ $thesis->doc_target_jurnal ? 'font-variation-settings:\'FILL\' 1' : '' }}">
                    {{ $thesis->doc_target_jurnal ? 'check_circle' : 'article' }}
                </span>
            </div>
            <div>
                <h3 class="text-base font-bold text-slate-900">Target Jurnal</h3>
                <p class="text-xs {{ $thesis->doc_target_jurnal ? 'text-emerald-600' : 'text-slate-400' }}">
                    {{ $thesis->doc_target_jurnal ? 'Sudah diunggah — dapat diganti dengan file baru' : 'Belum diunggah' }}
                </p>
            </div>
        </div>

        <form action="{{ route('mahasiswa.upload.target-jurnal') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            <div x-data="{ fileName: '' }" class="relative">
                <label class="block text-xs font-semibold text-slate-600 mb-2">File Target Jurnal (PDF)</label>
                <div class="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:border-slate-400 hover:bg-slate-50 transition cursor-pointer relative">
                    <input type="file" name="doc_target_jurnal" accept=".pdf" required
                        class="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                        @change="fileName = $event.target.files[0] ? $event.target.files[0].name : ''">
                    <span class="material-symbols-outlined text-3xl text-slate-300 mb-2">article</span>
    ...
    <div class="mt-4 p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
        ...
        <a href="{{ $thesis->getDownloadUrl('target_jurnal') }}" target="_blank" class="flex items-center gap-1.5 text-xs text-blue-700 font-semibold hover:underline">
