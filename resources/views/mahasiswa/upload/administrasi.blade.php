...
@section('title', 'Tahap 1: Administrasi')
...
<div class="space-y-6" x-data="{ 
    ...
    <header class="relative p-8 bg-white rounded-[32px] shadow-premium-sm border border-outline-variant/10 overflow-hidden group">
        ...
        <div class="relative z-10">
            <nav class="flex items-center gap-2 text-[10px] font-black text-on-surface-variant/50 mb-3 uppercase tracking-[0.2em]">
                ...
                <span class="text-primary font-black">Tahap 1</span>
            </nav>
            <h1 class="text-3xl font-black tracking-tighter text-on-surface mb-2 leading-tight">Berkas Administrasi</h1>
            <p class="text-on-surface-variant font-medium text-sm max-w-2xl leading-relaxed">
                Tahap awal verifikasi dokumen penugasan dospem dan izin penelitian.
            </p>
        </div>
    </header>
    ...
    @if(session('status'))
        <div class="p-5 bg-success text-white rounded-2xl text-sm font-black flex items-center gap-4 shadow-lg shadow-success/20 animate-in fade-in slide-in-from-top-4">
            <span class="material-symbols-outlined text-2xl">check_circle</span> {{ session('status') }}
        </div>
    @endif

    <div class="premium-card p-8 bg-gradient-to-b from-surface-container-lowest to-white border-outline-variant/10">
        <form action="{{ route('mahasiswa.upload.administrasi') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
                @foreach([
                    ['key' => 'doc_sk_pembimbing_1',   'label' => 'SK Pembimbing 1',       'icon' => 'assignment_ind'],
                    ['key' => 'doc_sk_pembimbing_2',   'label' => 'SK Pembimbing 2',       'icon' => 'assignment_ind'],
                    ['key' => 'doc_target_jurnal',     'label' => 'Judul & Target Jurnal', 'icon' => 'description'],
                    ['key' => 'doc_izin_penelitian',   'label' => 'Surat Izin Penelitian', 'icon' => 'badge'],
                ] as $doc)
                @php
                    $field = $doc['key'];
                    ...
                    $stateCfg = [
                        'empty' => [
                            ...
                            'icon' => 'cloud_upload',
                            'label' => 'Pilih Berkas (PDF/Word/Teks)',
                        ...
                        'pending' => [
                            ...
                            'icon' => 'hourglass_top',
                            'label' => 'Ganti Berkas?',
                            'statusText' => 'Sedang Diverifikasi Admin',
                            'statusColor' => 'text-warning'
                        ],
                        'accepted' => [
                            'bg' => 'bg-success/5 border-success/20 opacity-80',
                            'iconBg' => 'bg-success/10 text-success',
                            'icon' => 'verified',
                            'label' => 'Berkas Terverifikasi (Terkunci)',
                            'statusText' => 'Diterima & Valid',
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <label for="{{ $field }}" class="flex flex-col items-center justify-center p-8 border-2 border-dashed rounded-[24px] transition-all duration-500 group relative overflow-hidden
                            {{ $vStatus === 'accepted' ? 'cursor-not-allowed' : 'cursor-pointer hover:bg-white hover:border-primary/50' }}
                            {{ $cfg['bg'] }}">
                            ...
                            <div class="flex flex-col items-center gap-2">
                                <span class="text-[9px] text-on-surface-variant font-bold mt-1 opacity-60">File: {{ basename($path) }}</span>
                                <button type="button" @click.stop="previewUrl = '{{ $thesis->getDownloadUrl($folder) }}'; previewLabel = '{{ $doc['label'] }}'; previewExtension = '{{ $thesis->getFileExtension($folder) }}'; showPreviewModal = true"
                                    class="flex items-center gap-2 px-3 py-1.5 bg-white border border-outline-variant/10 rounded-full text-[9px] font-black uppercase tracking-widest text-on-surface hover:bg-primary hover:text-white transition-all shadow-sm">
                                    <span class="material-symbols-outlined text-sm">visibility</span> Lihat Berkas
                                </button>
                            </div>
                            @else
                            <span class="text-[9px] text-on-surface-variant font-bold mt-1 opacity-60">PDF/Word/Teks • Max 10MB</span>
                            @endif
                        </label>
                    </div>
                    ...
                    @if($vStatus === 'rejected')
                        <div class="p-4 bg-error text-white rounded-2xl flex gap-4 shadow-lg shadow-error/20 animate-in slide-in-from-bottom-2">
                            <span class="material-symbols-outlined shrink-0 text-2xl">info</span>
            ...
            <div class="pt-8 border-t border-outline-variant/10 flex flex-col sm:flex-row justify-between items-center gap-6">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-surface-container-high rounded-xl flex items-center justify-center text-on-surface-variant border border-outline-variant/10">
                        <span class="text-sm font-black">1/3</span>
                    </div>
                    <p class="text-[10px] font-bold text-on-surface uppercase tracking-widest">Tahap 1 Administrasi</p>
                </div>
                
                <button type="submit" class="w-full sm:w-auto px-10 py-4 bg-on-surface text-white rounded-2xl font-black shadow-xl shadow-on-surface/20 hover:bg-primary transition-all duration-300 hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-3 text-xs uppercase tracking-widest">
...
<script>
    function updateFileName(input) {
        ...
        if (input.files.length > 0) {
            ...
            nameDisplay.classList.add('text-primary');
            nameDisplay.closest('label').classList.add('ring-4', 'ring-primary/10', 'border-primary/50');
        }
    }
</script>
@endsection
