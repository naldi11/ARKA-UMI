...
@section('title', 'Upload SK Pembimbing 1')
...
<div class="max-w-2xl mx-auto">
    <div class="mb-7">
        <nav class="flex items-center gap-2 text-xs text-slate-400 mb-2 uppercase tracking-widest font-semibold">
            ...
            <span class="text-blue-700 font-bold">Upload SK Pembimbing 1</span>
        </nav>
        <h1 class="text-2xl font-black tracking-tight text-slate-900">Upload SK Pembimbing 1</h1>
        <p class="text-slate-500 text-sm mt-1">Unggah file SK Pembimbing 1 Anda (format PDF, maks. 10 MB).</p>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-7">
        <div class="flex items-center gap-3 mb-6 pb-5 border-b border-slate-100">
            <div class="w-10 h-10 {{ $thesis->doc_sk_pembimbing_1 ? 'bg-emerald-100' : 'bg-slate-100' }} rounded-xl flex items-center justify-center">
                <span class="material-symbols-outlined {{ $thesis->doc_sk_pembimbing_1 ? 'text-emerald-600' : 'text-slate-500' }} text-[22px]" style="{{ $thesis->doc_sk_pembimbing_1 ? 'font-variation-settings:\'FILL\' 1' : '' }}">
                    {{ $thesis->doc_sk_pembimbing_1 ? 'check_circle' : 'assignment_ind' }}
                </span>
            </div>
            <div>
                <h3 class="text-base font-bold text-slate-900">SK Pembimbing 1</h3>
                <p class="text-xs {{ $thesis->doc_sk_pembimbing_1 ? 'text-emerald-600' : 'text-slate-400' }}">
                    {{ $thesis->doc_sk_pembimbing_1 ? 'Sudah diunggah — dapat diganti dengan file baru' : 'Belum diunggah' }}
                </p>
            </div>
        </div>

        <form action="{{ route('mahasiswa.upload.sk-pembimbing-1') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            <div x-data="{ fileName: '' }" class="relative">
                <label class="block text-xs font-semibold text-slate-600 mb-2">File SK Pembimbing 1 (PDF)</label>
                <div class="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:border-slate-400 hover:bg-slate-50 transition cursor-pointer relative">
                    <input type="file" name="doc_sk_pembimbing_1" accept=".pdf" required
    ...
    <div class="mt-4 p-4 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-between">
        ...
        <a href="{{ $thesis->getDownloadUrl('sk_pembimbing_1') }}" target="_blank" class="flex items-center gap-1.5 text-xs text-blue-700 font-semibold hover:underline">
