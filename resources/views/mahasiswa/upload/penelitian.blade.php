...
@section('title', 'Tahap 2: Penelitian')
...
<div class="space-y-6" x-data="{ 
    ...
    <header class="relative p-8 bg-white rounded-[32px] shadow-premium-sm border border-outline-variant/10 overflow-hidden group">
        <div class="absolute -right-20 -top-20 w-80 h-80 bg-accent-indigo/5 rounded-full blur-3xl group-hover:bg-accent-indigo/10 transition-all duration-700"></div>
        <div class="relative z-10">
            <nav class="flex items-center gap-2 text-[10px] font-black text-on-surface-variant/50 mb-3 uppercase tracking-[0.2em]">
                ...
                <span class="text-accent-indigo font-black">Tahap 2</span>
            </nav>
            <h1 class="text-3xl font-black tracking-tighter text-on-surface mb-2 leading-tight">Jurnal & Skripsi</h1>
            <p class="text-on-surface-variant font-medium text-sm max-w-2xl leading-relaxed">
                Tahap inti penelitian. Unggah draf jurnal dan naskah skripsi final Anda.
            </p>
        </div>
    </header>

    <div class="premium-card p-8 bg-gradient-to-b from-surface-container-lowest to-white border-outline-variant/10">
        <form action="{{ route('mahasiswa.upload.penelitian') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @foreach([
                    ['key' => 'doc_jurnal',  'label' => 'Jurnal Skripsi',         'icon' => 'article', 'max' => '10MB'],
                    ['key' => 'doc_skripsi', 'label' => 'Naskah Skripsi (PDF)', 'icon' => 'upload_file', 'max' => '20MB'],
                ] as $doc)
                @php
                    $field = $doc['key'];
                    ...
                    $stateCfg = [
                        'empty' => [
                            ...
                            'iconBg' => 'bg-accent-indigo/5 text-accent-indigo',
                            'icon' => 'picture_as_pdf',
                ...
                <div class="space-y-3">
                    ...
                    <div class="relative group">
                        ...
                        <label for="{{ $field }}" class="flex flex-col items-center justify-center p-10 border-2 border-dashed rounded-[24px] transition-all duration-500 group relative overflow-hidden
                            {{ $vStatus === 'accepted' ? 'cursor-not-allowed' : 'cursor-pointer hover:bg-white hover:border-accent-indigo/50' }}
                            {{ $cfg['bg'] }}">
                            ...
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <button type="button" @click.stop="previewUrl = '{{ $thesis->getDownloadUrl($folder) }}'; previewLabel = '{{ $doc['label'] }}'; previewExtension = '{{ $thesis->getFileExtension($folder) }}'; showPreviewModal = true"
                                    class="flex items-center gap-2 px-3 py-1.5 bg-white border border-outline-variant/10 rounded-full text-[9px] font-black uppercase tracking-widest text-on-surface hover:bg-accent-indigo hover:text-white transition-all shadow-sm">
                    ...
                    @if($vStatus === 'rejected')
                        <div class="p-4 bg-error text-white rounded-2xl flex gap-4 shadow-lg shadow-error/20 animate-in slide-in-from-bottom-2">
                            <span class="material-symbols-outlined shrink-0 text-2xl text-white">report_problem</span>
            ...
            <div class="pt-8 border-t border-outline-variant/10 flex flex-col sm:flex-row justify-between items-center gap-6">
                <a href="{{ route('mahasiswa.upload.administrasi.page') }}" class="flex items-center gap-3 px-6 py-4 text-xs font-black text-on-surface-variant hover:text-primary transition-all group uppercase tracking-widest">
                    <span class="material-symbols-outlined group-hover:-translate-x-2 transition-transform text-lg">arrow_back</span>
                    Kembali
                </a>
                
                <button type="submit" class="w-full sm:w-auto px-10 py-4 bg-accent-indigo text-white rounded-2xl font-black shadow-xl shadow-accent-indigo/20 hover:shadow-accent-indigo-lg transition-all duration-300 hover:-translate-y-1 active:scale-95 flex items-center justify-center gap-3 text-xs uppercase tracking-widest">
...
<script>
    function updateFileName(input) {
        ...
        if (input.files.length > 0) {
            ...
            nameDisplay.classList.add('text-accent-indigo');
            nameDisplay.closest('label').classList.add('ring-4', 'ring-accent-indigo/10', 'border-accent-indigo/50');
        }
    }
</script>
@endsection
